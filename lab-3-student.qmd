---
title: "Lab3: ggplot Data viz w/Rodents"
format: html
editor: source
execute: 
  echo: true
...
There are 30463 Rows and  15 Columns in the data frame. 

**2. What are the data types of the variables in this dataset?**

The variables record_id, month, day, year, plot_id, hindfoot_length,and weight are all considered numeric variables by R. species_id, sex, day_of_week, plot_type, genus, species, and taxa are all considered categorical variables. date is the only variable considered as a date type variable. 
...
```{r}
#| label: scatterplot_weight_hindfootlength

ggplot(data = surveys,
       mapping = aes(x= weight, y= hindfoot_length)) +
  geom_point(alpha= 0.5)+
  facet_wrap(~species)+
  labs(x= "Weight (g)", y= " ", title= "Species based Hind limb relationship with Weight",subtitle = "Length of the Hind Foot (mm)")
...
ggplot(data = surveys, aes(x= weight, y= species))+
  geom_jitter(color = "springgreen4",alpha = 0.5 )+
  geom_boxplot(outliers = FALSE)+#get rid of outliers
  labs(x= "Weight of Organisms (g)", y= "Species names", title= "Weight trends Among Species")
...
ggplot(data = surveys, aes(y= weight, x= species))+
  geom_jitter(color = "springgreen4",alpha = 0.5 )+
  geom_boxplot(outliers = FALSE, orientation = "x")+ #change box plots orientations 
  theme(axis.text.x = element_text(angle = 45))+#tilt axis labels
  labs(x= "Weight of Organisms (g)", y= "Species names", title= "Weight trends Among Species")
